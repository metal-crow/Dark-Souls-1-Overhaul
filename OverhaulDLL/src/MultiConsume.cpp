...
const static uint8_t item_decrement_original_bytes[6]                 = { 0x03, 0xD6, 0x38, 0xD0, 0x7E, 0x0C };
...
void __declspec(naked) __stdcall intercept_item_consumption()
{
    __asm
    {
        ...
        mov [edx], 1 // Left button string ID ("OK")
        mov edx, 0x12E33E8 //"DARKSOULS.exe"+0xEE33E8
        mov [edx], 2 // Right button string ID ("CANCEL")
        ...
        mov [eax+0x204], 1 // Player has max souls; limit usage to avoid wasting them
...
void __declspec(naked) __stdcall intercept_item_decrement()
{
    __asm
    {
        ...
        je adjust_item_dec
        cmp [ecx+4], 0x1F5 // 501 = Twin Humanities item ID
        je adjust_item_dec
        // Soul variants
        cmp [ecx+4], 0x190 // 400 = Soul of a Lost Undead item ID
        je adjust_item_dec
        cmp [ecx+4], 0x191 // 401 = Large Soul of a Lost Undead item ID
        je adjust_item_dec
        cmp [ecx+4], 0x192 // 402 = Soul of a Nameless Soldier item ID
        je adjust_item_dec
        cmp [ecx+4], 0x193 // 403 = Large Soul of a Nameless Soldier item ID
        je adjust_item_dec
        cmp [ecx+4], 0x194 // 404 = Soul of a Proud Knight item ID
        je adjust_item_dec
        cmp [ecx+4], 0x195 // 405 = Large Soul of a Proud Knight item ID
        je adjust_item_dec
        cmp [ecx+4], 0x196 // 406 = Soul of a Brave Warrior item ID
        je adjust_item_dec
        cmp [ecx+4], 0x197 // 407 = Large Soul of a Brave Warrior item ID
        je adjust_item_dec
        cmp [ecx+4], 0x198 // 408 = Soul of a Hero item ID
        je adjust_item_dec
        cmp [ecx+4], 0x199 // 409 = Soul of a Great Hero item ID
        je adjust_item_dec
...
void __declspec(naked) __stdcall soft_soul_correction()
{
    __asm
    {
        ...
        cmp [last_consumption_quantity], 1
        je originalcode_soft_soul_correction
