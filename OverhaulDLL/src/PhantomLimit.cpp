...
void Game::increase_phantom_limit1()
{
    ...
    *max_allowed_summons32 = max_allowed_summons8; // Reset max_allowed_summons32 back to original value
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xA63CBA;
    set_mem_protection(write_address, 5, MEM_PROTECT_RWX);
    memcpy_s(write_address, 5, patch1, 5);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x9B51EB;
    set_mem_protection(write_address, 2, MEM_PROTECT_RWX);
    memcpy_s(write_address, 2, patch3, 2);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x92A47C;
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x92A465;
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x9B1271;
    ...
    *max_allowed_summons32 = max_allowed_summons8; // Reset max_allowed_summons32 back to original value
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x8068A4;
    set_mem_protection(write_address, 3, MEM_PROTECT_RWX);
    ...
    *max_allowed_summons32 = max_allowed_summons8; // Reset max_allowed_summons32 back to original value
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x8068A9;
    set_mem_protection(write_address, 7, MEM_PROTECT_RWX);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x8037A7;
    set_mem_protection(write_address, 3, MEM_PROTECT_RWX);
    ...
    *max_allowed_summons32 = max_allowed_summons8; // Reset max_allowed_summons32 back to original value
    ...
    summon_char_types_callproc = (uint32_t)((uint8_t*)Game::ds1_base + 0x1629C0);

    // Init rest of the summon_char_types array to zero
    write_address = (uint8_t*)Game::ds1_base + 0x8068DF;
    ...
    write_address = (uint8_t*)Game::ds1_base + 0x322A00;
    set_mem_protection(write_address, 2, MEM_PROTECT_RWX);
    memcpy_s(write_address, 2, patch3, 2); // push 0x9
    ...
    */
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xEDE2CC;
    set_mem_protection(write_address, 4, MEM_PROTECT_RWX);
    ...
    *max_allowed_summons32 = max_allowed_summons8; // Reset max_allowed_summons32 back to original value
    write_address = (uint8_t*)Game::ds1_base + 0xABB55D;
    set_mem_protection(write_address, 5, MEM_PROTECT_RWX);
    ...
    players_connected_array_offsets_callproc = (uint32_t)((uint8_t*)Game::ds1_base + 0xAA2320);
    players_connected_array_offsets_returnhere = (uint32_t)((uint8_t*)Game::ds1_base + 0xAA24FF);

    write_address = (uint8_t*)Game::ds1_base + 0xAA24D2;
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xAA1A86;
    set_mem_protection(write_address, 7, MEM_PROTECT_RWX);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xAA2932;
    set_mem_protection(write_address, 7, MEM_PROTECT_RWX);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xAA1788;
    set_mem_protection(write_address, 3, MEM_PROTECT_RWX);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xA6615A;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret1, 1, pca_off1);

    write_address = (uint8_t*)Game::ds1_base + 0xA5B900;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    pca_off_jmp2 = (uint32_t)((uint8_t*)Game::ds1_base + 0xA5B939);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret2, 1, pca_off2);

    write_address = (uint8_t*)Game::ds1_base + 0x9C83DB;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret3, 2, pca_off3);

    write_address = (uint8_t*)Game::ds1_base + 0x9C6BBE;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    pca_off_jmp4 = (uint32_t)((uint8_t*)Game::ds1_base + 0x9C6BDB);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret4, 1, pca_off4);

    write_address = (uint8_t*)Game::ds1_base + 0x92A44A;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    pca_off_jmp5 = (uint32_t)((uint8_t*)Game::ds1_base + 0x92A453);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret5, 1, pca_off5);

    write_address = (uint8_t*)Game::ds1_base + 0x9B53FB;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    pca_off_jmp6 = (uint32_t)((uint8_t*)Game::ds1_base + 0x9B5405);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret6, 1, pca_off6);

    write_address = (uint8_t*)Game::ds1_base + 0x93FEEE;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret7, 4, pca_off7);

    write_address = (uint8_t*)Game::ds1_base + 0xAA1771;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret8, 1, pca_off8);

    write_address = (uint8_t*)Game::ds1_base + 0xAA1791;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret9, 2, pca_off9);

    write_address = (uint8_t*)Game::ds1_base + 0xAA17AD;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret10, 1, pca_off10);

    write_address = (uint8_t*)Game::ds1_base + 0xAA29A9;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret11, 2, pca_off11);

    write_address = (uint8_t*)Game::ds1_base + 0xAA3141;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret12, 1, pca_off12);

    write_address = (uint8_t*)Game::ds1_base + 0xAA3268;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret13, 1, pca_off13);

    write_address = (uint8_t*)Game::ds1_base + 0xAA18E2;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret14, 0, pca_off14);

    write_address = (uint8_t*)Game::ds1_base + 0xAA18E7;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret15, 1, pca_off15);

    write_address = (uint8_t*)Game::ds1_base + 0xAA2748;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    pca_off_jmp16 = (uint32_t)((uint8_t*)Game::ds1_base + 0xAA27A9);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret16, 1, pca_off16);

    write_address = (uint8_t*)Game::ds1_base + 0xAA27A1;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    inject_jmp_5b((uint8_t*)write_address, &pca_off_ret17, 2, pca_off17);

    write_address = (uint8_t*)Game::ds1_base + 0xAA2740;
    set_mem_protection(write_address, 16, MEM_PROTECT_RWX);
    ...
    write_address = (uint8_t*)Game::ds1_base + 0xAA277A;
    set_mem_protection(write_address, 6, MEM_PROTECT_RWX);
...
void Game::increase_phantom_limit2()
{
    print_console(Mod::output_prefix + "Increasing multiplayer phantom limit...");
